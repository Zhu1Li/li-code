...
- **目标漂移重锚**:长任务里模型容易被中间发现带跑偏(本来修 A、查着查着去重构 B),每步看局部都合理、累积起来就偏离原始需求。循环开始时锚定"最初的任务原文",之后**每隔 10 轮**注入一条 `<goal-check>` system-reminder 把原始目标贴回、要求模型自检"下一步是否仍服务于它、跑偏就重新对齐"。只追加尾部、不改历史(缓存安全);短任务(不足一个间隔)不触发、不打扰。这是三大 coding-agent 难点里"目标漂移"的正面应对。
